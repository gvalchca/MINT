...
def resid_description(res):
    if res:
        ch = str(res.get_parent().get_id()).strip()
        name = names(res.get_resname().strip())
        return ch + "|" + name + ":" + str(res.get_id()[1])
    else:
        return ""
    ...
    for i in range(0, len(h)):
        ...
        if tmp and not list((collections.Counter(tmp) &
            ...
            if len(h[i]) > 1:
                TMP.extend(tmp)
                vmd += str(i + 1) + "] " +\
                    helices_to_vmd([beg, end, beg_sec, end_sec]) + "\n"
                out += str(i + 1) + "] " + beg + "-" + end + " -> " +\
                    beg_sec + "-" + end_sec + "\n"
            else:
                TMP.extend(tmp)
                vmd += str(i + 1) + "] " +\
                    helices_to_vmd([beg, end]) + "\n"
                out += str(i + 1) + "] " + beg + " -> " + end_sec + "\n"
    return (out,  vmd)


def helices_to_vmd(nucs):
    tmp = [chain_and_number_from_nuc(nuc) for nuc in nucs]
    if len(nucs) == 4:
        out = "chain " + str(tmp[0][0]) + " and resid " + str(tmp[0][1]) +\
            " to " + str(tmp[1][1]) + "  " +\
            "chain "+str(tmp[2][0]) + " and resid " + str(tmp[2][1]) +\
            " to " + str(tmp[3][1])
    elif len(nucs) == 2:
        out = "chain " + str(tmp[0][0]) + " and resid " + str(tmp[0][1]) +\
            " to " + str(tmp[1][1])
...
def vmd_for_nuc(nuc):
    tmp = chain_and_number_from_nuc(nuc)
    if tmp[0].strip():
        return "(chain " + tmp[0] + " and resid " + tmp[1]+")"
    else:
        return "resid " + tmp[1]


def chain_and_number_from_nuc(nuc):
    tmp = nuc.split(":")
    nuc = tmp[1].split("-")[0]
    chain = tmp[0].split("|")[0]
    return (chain, nuc)
...
def for_vmd(d):
    if not isinstance(d, list):
        d = d.split("-")[:-1]
    i = 0
    previous_chain = chain_and_number_from_nuc(d[0])[0]
    while i < len(d):
        if previous_chain.strip():
            out = "chain " + previous_chain + " and resid "
        else:
            out = "resid"
        while (i < len(d) and
               chain_and_number_from_nuc(d[i])[0] == previous_chain):
            tmp = chain_and_number_from_nuc(d[i])
            out += " " + str(tmp[1])
            previous_chain = tmp[0]
            i += 1
        out += " "
        i += 1
    return out
...
def triplex_despription(t, nucleotides, if_print):
    ...
    for i in range(len(t)):
        ...
        vmd += str(i+1)+"]  " + for_vmd(triplex_tmp)
    return (out, vmd)
...
def analysis(megalist, nucleotides, if_print, IF_MODIFIED):
    ...
    if ps:
        ...
        vmd.append(pseudo_desc[1].split("\n")[1:])
    else:
        trajectory.append("")
        vmd.append([])
    if t:
        ...
        vmd.append(triplex_desc[1].split("\n")[1:])
    else:
        trajectory.append("")
        vmd.append([])
    if tmp[1]:
    ...
    out_dictionary = {"arches": arches,
                      ...
                      "dot_bracket": arches_to_dot_bracket(arches),
                      "fileText": f}
